...
import Data.Array.Accelerate.IO.Codec.BMP                 as A        -- package: accelerate-io
...
gaussianX = stencil (convolve9x1 gaussian9) A.clamp

gaussianY :: Acc (Image (RGB Float)) -> Acc (Image (RGB Float))
gaussianY = stencil (convolve1x9 gaussian9) A.clamp

-- <http://dev.theomader.com/gaussian-kernel-calculator/>
--
gaussian9 :: [Exp (RGB Float)]
...
main = do
  ...
  runBenchmarks opts rest
    [ bench "gaussian-blur" $ whnf (A.map packRGB) rgb_blur ]

  writeImageToBMP (name "rgb")  $ GPU.run $ A.map packRGB rgb_blur
