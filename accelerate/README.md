...
[![Docker](https://github.com/tmcdonell/accelerate-llvm/actions/workflows/docker-publish.yml/badge.svg)](https://github.com/tmcdonell/accelerate-llvm/actions/workflows/docker-publish.yml)
 ...
 * [`CUDA`](https://developer.nvidia.com/cuda-downloads) (if using the `accelerate-llvm-ptx` backend for NVIDIA GPUs)
    * Make sure that your GPU is [supported](https://en.wikipedia.org/wiki/CUDA#GPUs_supported) by the version of CUDA you have installed

Docker
------

A [docker](https://www.docker.com) container is available where this package is
built (using cabal) at `/accelerate-llvm`. Note that if you wish to use the
`accelerate-llvm-ptx` GPU backend you will need to install the [NVIDIA
docker](https://github.com/NVIDIA/nvidia-docker) tool.
